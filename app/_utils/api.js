...
export function useTVTrailer(id) {
  ...
  useEffect(() => {
    ...
    async function fetchTVTrailer() {
      try {
        const response = await fetch(`https://api.themoviedb.org/3/tv/${id}/videos?language=en-US&api_key=${API_KEY}`);
        if (!response.ok) {
          throw new Error('Failed to fetch trailer');
        }
        const data = await response.json();
        ...
        if (officialTrailer) {
          setTVTrailerUrl(`https://www.youtube.com/embed/${officialTrailer.key}`);
        } else {
